...
class flexmlsConnectSettings {
	...
	function settings_init() {
		...
		if ($options['search_results_fields'] == '' || count($options['search_results_fields']) == 0 ) {
...
// 		update_option('permalink_structure', get_option('permalink_structure'));
		if (empty($options['oauth_key']) && empty($options['oauth_secret']) && empty($options['oauth_failure'])){
			...
			$oauth_info = $fmc_api->CreateOauthKey($data);
			if (!$oauth_info){
...
//		echo "</td></tr></table>";
	...
	function settings_overview_search_results_page() {
		echo "";
	}
...
function settings_field_listing() {
	...
	function settings_field_cart_checkbox() {
		$options = get_option('fmc_settings');
		$checked_code = " checked='checked'";
		...
		echo "<label><input type='checkbox' name='fmc_settings[portal_carts]' value=true {$listing_checked_yes} /> Enable favorites, possibilities, and rejects on search results and detail pages</label><br/><br/>";

	}

    function settings_field_portal_checkboxes() {
        $options = get_option('fmc_settings');
        $checked_code = " checked='checked'";
        ...
        echo "<label><input id='summary-views-check' type='checkbox' name='fmc_settings[portal_search]' value=true {$search_checked_yes} /> On Search Results</label><br/>";
        echo "<label><input id='detail-views-check' type='checkbox' name='fmc_settings[portal_listing]' value=true {$listing_checked_yes} /> On Listing Details</label><br/><br/>";

     }

    function settings_field_portal_numbers() {
        $options = get_option('fmc_settings');
        $portal_mins = "\"\"";
        $detail_page = "\"\"";
        $search_page = "\"\"";
        ...
        if ($options['search_page'] !==null){
         ...
         echo "<label> After
             <input name='fmc_settings[portal_mins]' value={$portal_mins} style='width:25px' />
             minute(s) have passed</label><br/>";

         echo "<label> After
               <input id='detail-views' name='fmc_settings[detail_page]' value={$detail_page} style='width:25px' />
               listing details have been viewed</label><br/>";

         echo "<label> After
                <input id='summary-views' name='fmc_settings[search_page]' value={$search_page} style='width:25px' />
                listing summary pages have been viewed</label><br/>";


		$checked_code = " checked='checked'";

		if (array_key_exists('portal_force', $options) and $options['portal_force'] === true) {
			$force_checked_yes = $checked_code;
		}
		else {
		    $force_checked_yes = '';
		}

		echo "<label><input type='checkbox' name='fmc_settings[portal_force]' value=true {$force_checked_yes} /> Force users to register/log-in</label><br/>";
    ...
    function settings_field_portal_location() {
        $options = get_option('fmc_settings');
	...
	function settings_field_search_results_fields() {
		global $fmc_api;
		$options = get_option('fmc_settings');
	...
	function validate_search_results_fields($input) {
		$valid_fields = array();
		if(count($input) > 0) {
			global $fmc_api;
			$api_property_fields = $fmc_api->GetStandardFields();
